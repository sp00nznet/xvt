...
COND_JUMPS = {
    ...
    'jp', 'jnp', 'jcxz', 'jecxz',
    'loop', 'loope', 'loopne',   # loop's target must be a block leader too
}
...
class Disassembler:
    ...
    def find_functions(self, code_start: int, code_end: int, iat_map: dict = None) -> dict:
        ...
        # Recursive descent to closure: the linear E8 scan skips 5 bytes after
        # every hit, so a call preceded by a stray 0xE8 byte is missed. Feed
        # each disassembled function's own call targets back in until nothing
        # new appears — otherwise those callees never get lifted and the link
        # fails on them.
        pending = sorted_targets
        while pending:
            for i, addr in enumerate(pending):
                if i % 1000 == 0 and i > 0:
                    print(f"[*] Disassembling function {i}/{len(pending)}...")
            ...
            for func in functions.values():
                discovered |= func.calls_to
            pending = sorted(t for t in discovered
                             if t not in functions and code_start <= t < code_end)
            if pending:
                print(f"[*] Closure pass: {len(pending)} newly referenced functions")
