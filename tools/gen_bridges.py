...
FUNC_ARGS = {
    ...
    # --- XvT additions ---
    'CompareStringW': 6, 'DebugBreak': 0, 'DeleteFileA': 1, 'ExitThread': 1,
    'FatalAppExitA': 2, 'GetCurrentThread': 0, 'GetSystemTime': 1,
    'GetThreadPriority': 1, 'GetTimeZoneInformation': 1,
    'GetVolumeInformationA': 8, 'HeapValidate': 3, 'IsBadWritePtr': 2,
    'LocalAlloc': 2, 'LocalFree': 1, 'ResumeThread': 1,
    'SetConsoleCtrlHandler': 2, 'SetThreadPriority': 2, 'SuspendThread': 1,
    'TerminateThread': 2, 'TlsFree': 1,
    'CreateCompatibleDC': 1, 'GetObjectA': 3, 'GetPixel': 3, 'SetPixel': 4,
    'StretchBlt': 11, 'LoadImageA': 6, 'MessageBeep': 1,
    'acmStreamClose': 2, 'acmStreamConvert': 3, 'acmStreamPrepareHeader': 3,
    'acmStreamSize': 4, 'acmStreamUnprepareHeader': 3,
    'auxGetVolume': 2, 'waveInAddBuffer': 3, 'waveInClose': 1,
    'waveInReset': 1, 'waveInStart': 1, 'waveInStop': 1,
    'DirectDrawCreate': -2,
        ...
        for va, name, ordinal in sorted(dll_groups[dll]):
            ...
            if nargs == -2:
                ...
                if nargs == 0:
                    ...
                    lines.append(f'    if (!fn) fn = (STDFN0)GetProcAddress(LoadLibraryA("{dll}"), "{name}");')
                    lines.append(f'    if (fn) g_eax = fn();')
                    lines.append(f'    g_esp += 4;')
                else:
                    lines.append(f'    static STDFN{nargs} fn = NULL;')
                    lines.append(f'    if (!fn) fn = (STDFN{nargs})GetProcAddress(LoadLibraryA("{dll}"), "{name}");')
                    # Read args
