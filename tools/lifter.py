...
class Lifter:
    ...
    def _make_condition(self, jcc_mnemonic: str) -> str:
        ...
        if setter == 'cmp':
            ...
            # Shifts set ZF/SF/PF from the result; ops is "<dest>, 0" and dest
            # already holds the shifted value, so compare it against zero.
            return f"/* shift result */ {cmp_macro}({ops})"
    ...
    def lift_instruction(self, insn) -> list:
        ...
        # --- Shifts ---
            ...
            if len(ops) == 2:
                ...
                b = self._fmt_read(ops[1])
                lines.append(f"{self._fmt_write(ops[0], f'{a} << {b}')}; {comment}")
                self._set_flag_state(lines, 'shift', f"{a}, 0")
        ...
        # --- Misc ---
            ...
            lines.append(f"; /* int3 breakpoint */ {comment}")  # null stmt: an int3-only block is still a labeled statement
    ...
    def lift_function(self, func) -> str:
        ...
        # A goto whose label was never emitted means the branch target fell
        # outside the discovered blocks (jump into data / mid-instruction, from
        # a function-finder false positive). Emit a trap label so the output
        # always compiles and reaching it is visible at runtime.
        body = '\n'.join(body_lines)
        emitted = set(re.findall(r'^(L_[0-9A-F]{8}):', body, re.M))
        for lbl in sorted(set(re.findall(r'goto (L_[0-9A-F]{8})', body)) - emitted):
            lines.append(f'{lbl}: /* target outside discovered blocks - not real code */')
            lines.append(f'    fprintf(stderr, "!!! {name}: jumped to unmapped {lbl}\\n");')
            lines.append(f'    esp += 4; return;')
