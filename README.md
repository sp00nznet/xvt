...
The [XWA recomp](https://github.com/sp00nznet/xwa) has drilled down plenty: full boot, concourse rendering, screen-to-screen menu navigation, and a complete instruction-level map of the flight-entry path. It now carries the engine's own mission route into flight and **submits real geometry** — 230 meshes and ~3,950 vertices per frame, parsed from the actual `.tie` rather than fabricated by a force-launch path. The open problem there is *placement*: every object reports position `(0,0,0)`, so the whole mission is stacked at the origin and projects off-screen. Arrival placement is the live question.
...
- **Illuminate the XWA blocker** — XvT's single-player craft spawn is **local**, with no DirectPlay involved (`docs/directplay.md`, `docs/singleplayer-spawn.md`), and the spawn loop `sub_00409390` writes a complete per-craft recipe: flight-group stride `0x23`, object record at `fg+0x1F` with stride `0xEC`, active tag `fg+0x1A = 2`, render status `obj+0x2CA ∈ {2,4}`, and — the part that matters most right now — **arrival placement**, `fg+0x10`/`fg+0x12` position components with RNG scatter and an orientation word at `obj+0x77`.

  Honesty about how that hypothesis has fared: the half of it that generalised to XWA was the *field recipe*. The half that did not was the inference that XWA's DirectPlay route must be the wrong path for single player. XWA has since established that it genuinely does run even a single-player mission through a **local** DirectPlay session (session name = the mission path with a `~` prefix), and that world build refuses to run until that session create returns non-zero — the fix there was to make the create take its host-with-one-player short path, not to bypass DirectPlay. The two engines diverge on this: XvT's DirectPlay really is multiplayer-only, XWA's is a single-player gate as well. Predecessor and successor are not the same program, and this is where assuming they were would have cost time.
